...
if __name__ == '__main__':
    ...
    # %% Read restart data
    # Read last simulation results
    # NOTE: Specify the last cycle start time
    last_cycle_d = 10
    last_cycle_start_time = timestamp_list[last_cycle_d]
    # last_cycle_start_time = datetime(2018, 10, 28)

    filename = f'nygrid_sim_{sim_name}_{last_cycle_start_time.strftime("%Y%m%d")}_{valid_days}_{lookahead_days}.pkl'
    with open(os.path.join(sim_results_dir, filename), 'rb') as f:
        nygrid_results = pickle.load(f)
    logging.info(f'Read last cycle nygrid_results in {filename}')
    
    # Set initial conditions for the next iteration
    time_before_next_cycle = last_cycle_start_time + timedelta(hours=valid_hours)
    ...
    for d in tqdm(range(last_cycle_d+1, len(timestamp_list)), desc='Running OPF'):
        t = time.time()
