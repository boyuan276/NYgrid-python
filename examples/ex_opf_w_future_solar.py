...
def read_renewable_data(solar_data_dir, onshore_wind_data_dir, offshore_wind_data_dir):
    ...
    return current_solar_gen_agg, onshore_wind_gen_agg, offshore_wind_gen_agg, future_solar_gen_agg


def update_load_profile(load_profile, current_solar_gen_agg=None, onshore_wind_gen_agg=None, future_solar_gen_agg=None):
    # Treat renewable generation as negative load
    ...
    return load_profile_renewable


def read_grid_data(grid_data_dir, start_date):
    # Read load profile
    ...
    gen_max_profile = pd.read_csv(os.path.join(grid_data_dir, f'genmax_profile_{start_date.year}.csv'),
                                  parse_dates=['TimeStamp'], index_col='TimeStamp')
    gen_max_profile.index.freq = 'H'

    gen_min_profile = pd.read_csv(os.path.join(grid_data_dir, f'genmin_profile_{start_date.year}.csv'),
                                  parse_dates=['TimeStamp'], index_col='TimeStamp')
    gen_min_profile.index.freq = 'H'

    # Read generator ramp rate profile
    gen_ramp30_profile = pd.read_csv(os.path.join(grid_data_dir, f'genramp30_profile_{start_date.year}.csv'),
                                     parse_dates=['TimeStamp'], index_col='TimeStamp')
    gen_ramp30_profile.index.freq = 'H'

    # Read generator cost profile (linear)
    gen_cost0_profile = pd.read_csv(os.path.join(grid_data_dir, f'gencost0_profile_{start_date.year}.csv'),
                                    parse_dates=['TimeStamp'], index_col='TimeStamp')
    gen_cost0_profile.index.freq = 'H'

    gen_cost1_profile = pd.read_csv(os.path.join(grid_data_dir, f'gencost1_profile_{start_date.year}.csv'),
                                    parse_dates=['TimeStamp'], index_col='TimeStamp')
    gen_cost1_profile.index.freq = 'H'

    return (load_profile, gen_profile, gen_max_profile, gen_min_profile,
            gen_ramp30_profile, gen_cost0_profile, gen_cost1_profile)
...
if __name__ == '__main__':
    ...
    # Start timer
    t = time.time()
    logging.info(f"Starting program at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if not os.path.exists(os.path.join(results_dir, 'w_solar')):
        os.mkdir(os.path.join(results_dir, 'w_solar'))
    ...
    logging.info('Onshore wind data directory: {}'.format(onshore_wind_data_dir))

    offshore_wind_data_dir = os.path.join(data_dir, 'offshore_wind')
    logging.info('Offshore wind data directory: {}'.format(offshore_wind_data_dir))
    ...
    load_profile, gen_profile, genmax_profile, genmin_profile, genramp30_profile, \
        gencost0_profile, gencost1_profile = read_grid_data(grid_data_dir, start_date)

    # %% Read renewable generation data

    current_solar_gen_agg, onshore_wind_gen_agg, offshore_wind_gen_agg, future_solar_gen_agg = \
        read_renewable_data(solar_data_dir, onshore_wind_data_dir, offshore_wind_data_dir)

    # %% Update load profile

    load_profile_renewable = update_load_profile(load_profile, current_solar_gen_agg,
                                                 onshore_wind_gen_agg, future_solar_gen_agg)
    ...
    for d in range(len(timestamp_list) - 1):
        ...
        if d == 0:
            last_gen = None
            logging.info('No initial condition.')
        else:
            last_gen = nygrid_sim.get_last_gen(model_multi_opf)
            logging.info('Initial condition set from previous day.')
        ...
        if check_status(results_multi_opf):
            logging.info(f'Objective: {model_multi_opf.obj():.4e}')

            # %% Process results
            results = nygrid_sim.get_results_multi_opf(model_multi_opf)
            logging.info(f'Cost: {results["COST"]:.4e}')
            ...
            filename = f'thermal_w_solar_{start_datetime.strftime("%Y%m%d%H")}_{end_datetime.strftime("%Y%m%d%H")}.csv'
            thermal_pg.to_csv(os.path.join(results_dir, 'w_solar', filename))
            logging.info(f'Saved thermal generation results in {filename}')

            # Save simulation results to pickle
            filename = f'nygrid_sim_w_solar_{start_datetime.strftime("%Y%m%d%H")}_{end_datetime.strftime("%Y%m%d%H")}.pkl'
            with open(os.path.join(results_dir, 'w_solar', filename), 'wb') as f:
                pickle.dump([nygrid_sim, model_multi_opf, results], f)
            logging.info(f'Saved simulation results in {filename}')
            elapsed = time.time() - t
