...
if not os.path.exists(os.path.join(results_dir, 'w_renew_elec')):
    os.mkdir(os.path.join(results_dir, 'w_renew_elec'))
...
res_load_change_agg = res_load_change.groupby(groupby_dict, axis=1).sum()
res_load_change_agg = res_load_change_agg/1e3 # convert from kW to MW

# Scale down residential load change by 50%
res_load_change_agg = res_load_change_agg*0.5
...
# Load profile with residential load change
res_load_change_agg.index = current_solar_gen_agg.index
load_profile_renewable = load_profile_renewable.add(res_load_change_agg, fill_value=0)
...
for d in range(len(timestamp_list)-1):
    ...
    if nygrid_sim.check_status(results_multi_opf):
        ...
        thermal_pg.to_csv(os.path.join(results_dir, 'w_renew_elec', filename))
        ...
        with open(os.path.join(results_dir, 'w_renew_elec', filename), 'wb') as f:
